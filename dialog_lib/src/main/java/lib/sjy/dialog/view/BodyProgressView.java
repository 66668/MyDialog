...
class BodyProgressView extends ScaleLinearLayout {
    ...
    public BodyProgressView(Context context, AllParams params) {
        super(context);
        init(context, params);
    }
    ...
    private void init(Context context, AllParams params) {
        ...
        //如果没有背景色，则使用默认色
        int backgroundColor = mProgressParams.backgroundColor != 0 ? mProgressParams.backgroundColor : DialogColor.common_bg;
        ...
        if (mProgressParams.style == ProgressParams.STYLE_HORIZONTAL) {
            ...
            mProgressParams.progressHeight = DialogDimens.PROGRESS_HEIGHT_HORIZONTAL;
        }
        //旋转样式
        else {
            if (progressDrawableId != 0) {
            ...
            mProgressParams.progressHeight = DialogDimens.PROGRESS_HEIGHT_SPINNER;
        }
